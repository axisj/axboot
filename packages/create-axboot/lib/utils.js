...
import { defaultPackageManager, lockfileNames, packageManagers, recommendedTemplate, templatesDir, } from "./types.js";
...
export async function getAppName(reqName, rootDir) {
...
export function isValidGitRepoUrl(gitRepoUrl) {
    return ["https://", "git@"].some(item => gitRepoUrl.startsWith(item));
}
export async function getGitCommand(gitStrategy) {
    switch (gitStrategy) {
        case "shallow":
        case "copy":
...
export function escapeShellArg(s) {
    let res = `'${s.replace(/'/g, "'\\''")}'`;
    res = res.replace(/^(?:'')+/g, "").replace(/\\'''/g, "\\'");
    return res;
}
export async function copyTemplate(template, dest, language) {
...
export async function getPackageManager(dest, { packageManager, skipInstall }) {
