export const bizNumFormatter = (value: any) => {
...
export const bizNumParser = (value: any) => {
  return `${value}`!.replace(/[^0-9]/g, "");
};

export const phoneNumFormatter = (value: any) => {
  let str = `${value}`;
  if (str.startsWith("0.")) {
    str = str.replace(/^0./, "0");
  }
...
export const phoneNumParser = (value: any) => {
  return `${value}`!.replace(/[^0-9]/g, "");
};
